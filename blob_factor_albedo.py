...
def get_one_blob(mixtures, parent_path, scale_z,idx):
    ...
    print("================ ", scale_z, " ", torch.sqrt(cov1d- blur_margin).item(), " ================")
    ...
    transmittance = torch.exp(-mix_extinction_RGB * torch.sqrt(cov1d - blur_margin)[:,None] * 6).mean(dim=1)
...
def thickness_factor_optimization(mixtures, parent_path, scale_z,idx):
    ...
    factor = torch.tensor([[1.0], [1.0]], dtype=torch.float32, requires_grad=True, device="cuda")
...
def read_rand_mixtures(idx):
    assert idx <= 4, "the idx should be smaller than 4"
    ...
    folders = sorted([f for f in os.listdir('blob_factor_km/data') if os.path.isdir(os.path.join('blob_factor_km/data', f)) and "rand" in f], key = extract_number)

    mixtures = []
    for f in folders:
        path = os.path.join('blob_factor_km/data', f)
    ...
    mixtures9 = mixtures[(int(idx) - 1 )* 9: int(idx)*9]
    folders9 = folders[(int(idx) - 1 )* 9: int(idx)*9]



    return mixtures9, folders9

def get_standard_mixtures(idx):

    assert idx <= 5, "the idx should be smaller than 5"
    ...
    return mixtures13, string_representation


def get_pure_cmyk_mixtures():
    mixtures = np.array([
        [100. ,   0. ,   0. ,   0. ,   0.  ,   0.],
        [  0. , 100. ,   0. ,   0. ,   0.  ,   0.],
        [  0. ,   0. , 100. ,   0. ,   0.  ,   0.],
    ...
    folders = ["cyan", "magenta", "yellow", "black"]

    return mixtures, folders
